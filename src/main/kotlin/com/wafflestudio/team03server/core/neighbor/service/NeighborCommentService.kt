...
class NeighborCommentServiceImpl(
    ...
    override fun createNeighborComment(userId: Long, postId: Long, createNeighborCommentRequest: CreateNeighborCommentRequest) {
        val commenter = getUserById(userId)
        val post = getNeighborPostById(postId)
        val comment = createNeighborCommentRequest.comment
        val neighborComment = NeighborComment(neighborPost = post, commenter = commenter, comment = comment)
        neighborCommentRepository.save(neighborComment)
    }

    private fun getCommentById(commentId: Long) = neighborCommentRepository.findByIdOrNull(commentId) ?: throw Exception404("${commentId}에 해당하는 댓글이 없습니다.")
    ...
    override fun updateNeighborComment(userId: Long, commentId: Long, updateNeighborCommentRequest: UpdateNeighborCommentRequest) {
